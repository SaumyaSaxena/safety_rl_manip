...
import time
import json
from .DDPG_core import TransformerActorCritic
import wandb
...
class DDPGMultimodal(torch.nn.Module):
    """
    ...
    def update(self, data, epoch):
        ...
        q_sample = self.ac.forward_value(transformer_outputs, data['act'], train=True)
        action_pred = self.ac.forward_action(transformer_outputs, train=True)
        ...
        for p in self.ac.get_q_parameters():
            p.requires_grad = False
        q_policy = self.ac.forward_value(transformer_outputs, action_pred[:,:,0,:], train=True) # TODO: considering the action at current time step
        outputs['q_policy'] = q_policy[:,-1,0,0]
