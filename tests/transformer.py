...
class MultiHeadAttentionRoPE(nn.Module):
    def __init__(self, model_dim, num_heads, max_seq_len, masked):
        ...
        # --- RoPE additions ---
        # We'll build cos and sin up to max_seq_len length
        cos, sin = build_rotary_pos_emb(seq_len=max_seq_len, dim=self.split_head_dim)
        self.register_buffer("rope_cos", cos)  # shape => (max_seq_len, split_head_dim//2)
        self.register_buffer("rope_sin", sin)  # shape => (max_seq_len, split_head_dim//2)
...
class SpatialContrastiveEmbeddingModel(nn.Module):
    def __init__(self, num_embeddings, embedding_dim):
        super(SpatialContrastiveEmbeddingModel, self).__init__()
...
class GPTLayer_CA(nn.Module):
    def __init__(self, model_dim, num_heads, max_seq_len, dim_ff, dropout, masked, mha):
        super(GPTLayer_CA, self).__init__()
        self.self_attention = mha(model_dim, num_heads, max_seq_len, masked=masked)
        self.cross_attention = mha(model_dim, num_heads, max_seq_len, masked=masked)
...
class GPTLayer_SA(nn.Module):
    def __init__(self, model_dim, num_heads, max_seq_len, dim_ff, dropout, masked, mha):
        ...
        self.self_attention = mha(2*model_dim, num_heads, max_seq_len, masked=masked)
...
class GPT(nn.Module):
    def __init__(self, state_dim, model_dim, action_dim, num_heads, max_seq_len, dim_ff, pos_embedding, dropout, n_layers, mha, critic=False, masked=True, gauss=False, SA=False):
        ...
        if self.SA:
            self.decoder_layers = nn.ModuleList([GPTLayer_SA(model_dim, num_heads, max_seq_len, dim_ff, dropout, masked, mha) for _ in range(n_layers)])
        else:
            self.decoder_layers = nn.ModuleList([GPTLayer_CA(model_dim, num_heads, max_seq_len, dim_ff, dropout, masked, mha) for _ in range(n_layers)])
        self.critic = critic
        self.gauss = gauss
        if self.critic:
...
class AdaLNLayer(nn.Module):
    ...
    def __init__(self, model_dim, num_heads, max_seq_len, dim_ff, dropout, masked, mha):
        super().__init__()
        self.attn = mha(model_dim, num_heads, max_seq_len, masked=masked)
...
class GPT_AdaLN(nn.Module):
    def __init__(self, state_dim, model_dim, action_dim, num_heads, max_seq_len, dim_ff, pos_embedding, dropout, n_layers, mha, critic=False, masked=True, gauss=False):
        ...
        self.decoder_layers = nn.ModuleList([AdaLNLayer(model_dim, num_heads, max_seq_len, dim_ff, dropout, masked, mha) for _ in range(n_layers)])
        self.critic = critic
        self.gauss = gauss
        if self.critic:
...
class Transformer(nn.Module):
    def __init__(self, hp_dict, max_seq_len):
        ...
        if hp_dict['pos_embed'] == "SPE":
            ...
            self.pos_embedding = SpatialContrastiveEmbeddingModel(self.max_seq_len, embedding_dim=256)
            self.pos_embedding.load_state_dict(
                torch.load(f"{self.hp_dict['SCE_embed_loc']}",
        ...
        if hp_dict["attn_mech"] == "AdaLN":
            self.decoder_actor = GPT_AdaLN(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_seq_len, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['actor'], mha, masked=hp_dict['masked'], gauss=hp_dict['gauss'])
            self.decoder_critic1 = GPT_AdaLN(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_seq_len, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'])
            self.decoder_critic2 = GPT_AdaLN(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_seq_len, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'])
        elif hp_dict["attn_mech"] in ["CA", "SA"]:
            sa_bool = False
            if hp_dict["attn_mech"] == "SA":
                sa_bool = True
            self.decoder_actor = GPT(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_seq_len, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['actor'], mha, masked=hp_dict['masked'], gauss=hp_dict['gauss'], SA=sa_bool)
            self.decoder_critic1 = GPT(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_seq_len, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'], SA=sa_bool)
            self.decoder_critic2 = GPT(hp_dict['state_dim'], hp_dict['model_dim'], self.action_dim, hp_dict['num_heads'], self.max_seq_len, hp_dict['dim_ff'], self.pos_embedding, hp_dict['dropout'], hp_dict['n_layers_dict']['critic'], mha, critic=True, masked=hp_dict['masked'], SA=sa_bool)
